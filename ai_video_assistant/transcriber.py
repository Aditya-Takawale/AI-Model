...
try:
    ...
    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
        logging.info("✅ Apple Silicon (Metal) GPU detected - will use Metal acceleration")
        DEVICE_TYPE = "mps"
    elif CUDA_AVAILABLE:
        ...
        logging.info(f"💻 Using CPU: {cpu_info}")
        DEVICE_TYPE = "cpu"
except ImportError:
    CUDA_AVAILABLE = False
    DEVICE_TYPE = "cpu"
    logging.warning("⚠️  PyTorch not imported properly - using CPU mode")
...
class AudioTranscriber:
    ...
    def transcribe(self, audio_path: str, language: str = None) -> dict:
        ...
        try:
            # Transcribe the audio with appropriate device
            if DEVICE_TYPE == "cuda":
                logger.info("🚀 Using GPU acceleration (CUDA)")
                options = {"fp16": True}
            elif DEVICE_TYPE == "mps":
                logger.info("� Using GPU acceleration (Apple Metal)")
                options = {"fp16": False}  # Metal doesn't support FP16
            else:
                logger.info("💻 Using CPU (slower, but works everywhere)")
                options = {"fp16": False}
